...
const STATUS_META: Record<string, { label: string; color: string }> = {
  ...
  invoiced:            { label: "Invoiced",            color: "rgba(124,92,252,0.15)"  },
  partially_invoiced:  { label: "Partial Invoice",     color: "rgba(79,142,247,0.15)"  },
        ...
        <table className="w-full min-w-[760px] text-[12px]">
          <thead>
            <tr className="border-b border-border bg-surface">
              {["SO #", "Date", "Customer", "Status", "Shipment Date", "Total", "Balance", "CRM"].map((h) => (
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((o) => (
                <tr key={o.id} className="border-t border-border hover:bg-surface-2 transition-colors">
                  ...
                  <td className="px-4 py-3 text-text max-w-[180px] truncate">{o.customer || "—"}</td>
                  ...
                  <td className="px-4 py-3 text-text-muted whitespace-nowrap">{fmtDate(o.shipment_date)}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {o.crm_deal_id ? (
                      <a
                        href={`https://crm.zoho.com/crm/tab/Potentials/${o.crm_deal_id}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-accent hover:underline text-[11px] font-medium"
                        title="Open CRM deal"
                      >
                        CRM ↗
                      </a>
